...
현재는 `/ready` 응답에도 trust proxy 진단이 포함되므로, 공개 URL에서 `trustProxy.ready=true`가 확인돼야 배포 게이트를 통과한 것으로 본다.
...
- public cluster + worker 분리 확장안과 1만 public read 기준 롤아웃 절차는 `docs/public-scale-rollout.md`를 참고한다
