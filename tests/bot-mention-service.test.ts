...
serialTest("mention collector follows the original permalink from extracted mention html when API relations are missing", async () => {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL) => {
    ...
    if (url.includes("/mention-1?")) {
      return createJsonResponse({
        id: "mention-1",
        username: "collectorbot",
        text: "@collectorbot #voiceAI #audio thanks",
        timestamp: "2026-03-25T12:00:00.000Z",
        permalink: "https://www.threads.com/@collectorbot/post/MENTION1"
      });
    }
    ...
    if (url === "https://www.threads.com/@collectorbot/post/MENTION1") {
      return new Response(
        `<!doctype html>
        <html lang="ko">
          <head>
            <link rel="canonical" href="https://www.threads.com/@collectorbot/post/MENTION1" />
            <meta property="og:title" content="collectorbot on Threads" />
            <meta property="og:description" content="@collectorbot #voiceAI #audio thanks" />
          </head>
          <body>
            <main>
              <article>
                <a href="https://www.threads.com/@collectorbot/post/MENTION1"><time datetime="2026-03-25T12:00:00.000Z">방금</time></a>
                <div>collectorbot</div>
                <div>@collectorbot #voiceAI #audio thanks</div>
                <a href="https://www.threads.com/@source/post/TARGET1">original</a>
              </article>
            </main>
          </body>
        </html>`,
    ...
    throw new Error(`Unexpected fetch URL in extracted mention html test: ${url}`);
  }) as typeof fetch;
  ...
  try {
    ...
    const sync = await collector.syncNow("test");
    assert.equal(sync.ok, true);
    assert.equal(sync.createdArchives, 1);
    assert.equal(data.botArchives[0]?.targetUrl, "https://www.threads.com/@source/post/TARGET1");
    assert.equal(data.botArchives[0]?.targetText, "Original source post body.");
    assert.equal(data.botArchives[0]?.noteText, "@collectorbot #voiceAI #audio thanks");
    assert.match(data.botArchives[0]?.markdownContent ?? "", /Author follow-up reply from the same thread\./);
  } finally {
    globalThis.fetch = previousFetch;
...
serialTest("mention collector ingests linked-user replies when bot mentions feed misses the comment", async () => {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.includes("/me/replies?")) {
      const accessToken = new URL(url).searchParams.get("access_token");
      if (accessToken !== "user-long-lived-token") {
        ...
        data: [
          {
            id: "mention-1",
            username: "linkedwriter",
            text: "감사합니다. Karpathy Loop @ss_threads_bot",
            timestamp: "2026-03-25T12:00:00.000Z",
            permalink: "https://www.threads.com/@linkedwriter/post/MENTION1",
            root_post: {
              id: "target-1",
    ...
    throw new Error(`Unexpected fetch URL in replies fallback test: ${url}`);
  }) as typeof fetch;
  ...
  try {
    ...
    assert.equal(data.botArchives[0]?.targetUrl, "https://www.threads.com/@source/post/TARGET1");
    assert.equal(data.botArchives[0]?.targetText, "Original source post body.");
    assert.equal(data.botArchives[0]?.noteText, "감사합니다. Karpathy Loop @ss_threads_bot");
  } finally {
    globalThis.fetch = previousFetch;
...
serialTest("mention collector repairs trigger-only archives from linked-user replies when the mention feed misses them", async () => {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    throw new Error(`Unexpected fetch URL in replies repair test: ${url}`);
  }) as typeof fetch;
  ...
  try {
    ...
    data.botArchives.push({
      ...
      mentionAuthorDisplayName: "Linked Writer",
      noteText: null,
      targetUrl: "https://www.threads.com/@linkedwriter/post/MENTION1",
      targetAuthorHandle: "linkedwriter",
      targetAuthorDisplayName: "Linked Writer",
      targetText: "감사합니다. Karpathy Loop @ss_threads_bot",
    ...
    const sync = await collector.syncNow("user_sync");
    assert.equal(sync.ok, true);
    assert.equal(sync.createdArchives, 0);
    assert.equal(sync.updatedArchives, 1);
    assert.equal(data.botArchives[0]?.targetUrl, "https://www.threads.com/@source/post/TARGET1");
    assert.equal(data.botArchives[0]?.targetText, "Original source post body.");
    assert.equal(data.botArchives[0]?.noteText, "감사합니다. Karpathy Loop @ss_threads_bot");
  } finally {
