...
async function renderReplySectionWithMessages(
  post: ExtractedPost,
...
async function buildMarkdownBody(
  ...
  const msg = await t();
  return await buildMarkdownBodyWithMessages(post, mediaRefs, warning, aiResult, msg);
}

async function buildMarkdownBodyWithMessages(
  post: ExtractedPost,
  mediaRefs: MarkdownMediaRefs,
  warning: string | null,
  aiResult: AiOrganizationResult | null,
