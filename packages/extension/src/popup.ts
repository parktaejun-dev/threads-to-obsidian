...
const cloudLinkButton = document.querySelector<HTMLButtonElement>("#cloud-link-button");
...
let cachedCloudConnection: CloudConnectionStatus | null = null;
  ...
  for (const button of recentList.querySelectorAll<HTMLButtonElement>("[data-delete-id]")) {
    button.addEventListener("click", async () => {
      ...
      if (saveButton) {
        saveButton.disabled =
          !state.supported ||
          (cachedSaveTarget === "notion" && !cachedNotionReady) ||
...
cloudLinkButton?.addEventListener("click", async () => {
  ...
  if (cachedCloudConnection?.state === "linked" || cachedCloudConnection?.state === "offline") {
    const state = (await chrome.runtime.sendMessage({ type: "disconnect-cloud-link" })) as PopupState;
