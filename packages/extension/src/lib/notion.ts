...
export interface NotionMediaPolicy {
...
export interface NotionSaveCoreOptions {
  token: string;
...
function resolveMessages(locale?: Locale): Messages | Promise<Messages> {
  return locale ? tSync(locale) : t();
}

function mapNotionError(status: number, body: NotionErrorResponse | null, retryAfter: string | null, msg: Messages): string {
...
async function notionRequest<T>(url: string, token: string, msg: Messages, init?: RequestInit): Promise<T> {
...
async function notionUploadRequest<T>(url: string, token: string, msg: Messages, formData: FormData): Promise<T> {
  const response = await fetch(url, {
    method: "POST",
    headers: {
      authorization: `Bearer ${token}`,
...
async function retrieveDataSource(token: string, dataSourceId: string, msg: Messages): Promise<NotionDataSourceResponse> {
  return await notionRequest<NotionDataSourceResponse>(`${NOTION_DATA_SOURCE_API_URL}/${dataSourceId}`, token, msg, {
    method: "GET"
  });
}
...
async function renderBundle(
  ...
  return await renderBundleFromAi(post, includeImages, inlineMedia, ai.result, ai.warning, locale);
}

async function renderBundleFromAi(
  post: ExtractedPost,
  includeImages: boolean,
  inlineMedia: boolean,
  aiResult: AiOrganizationResult | null,
...
async function createFileUpload(
  ...
  return await notionRequest<NotionFileUploadResponse>(NOTION_FILE_UPLOAD_API_URL, token, msg, {
    method: "POST",
    body: JSON.stringify({
...
async function completeFileUpload(token: string, fileUploadId: string, msg: Messages): Promise<void> {
  await notionRequest<NotionFileUploadResponse>(`${NOTION_FILE_UPLOAD_API_URL}/${fileUploadId}/complete`, token, msg, {
    method: "POST",
    body: JSON.stringify({})
  });
}

async function uploadBlobToNotion(token: string, blob: Blob, filename: string, msg: Messages): Promise<string> {
  ...
  if (numberOfParts === 1) {
    ...
    await notionUploadRequest<NotionFileUploadResponse>(uploadUrl, token, msg, formData);
...
async function uploadRemoteMediaFile(
  token: string,
  url: string,
  label: string,
  kind: "image" | "video",
  msg: Messages
): Promise<UploadedNotionMediaFile> {
...
async function uploadReplyMediaFiles(
  token: string,
  reply: AuthorReply,
  replyIndex: number,
  msg: Messages
...
async function appendBlockChildren(token: string, blockId: string, children: Record<string, unknown>[], msg: Messages): Promise<void> {
  if (children.length === 0) {
    return;
  }

  await notionRequest<Record<string, unknown>>(`${NOTION_BLOCKS_API_URL}/${blockId}/children`, token, msg, {
    method: "PATCH",
    body: JSON.stringify({
      children
    })
...
export async function savePostToNotionCore(
  post: ExtractedPost,
  options: NotionSaveCoreOptions
): Promise<NotionSaveResult> {
  const msg = await resolveMessages(options.locale);
  ...
  const responseBody = await notionRequest<NotionCreatePageResponse>(NOTION_API_URL, token, msg, {
    method: "POST",
    body: JSON.stringify({
      parent,
...
export async function savePostToNotion(
  ...
  const ai = aiOrganization ? await organizePostWithAi(post, aiOrganization) : { result: null, warning: null };
  return await savePostToNotionCore(post, {
    token,
    parentType: settings.parentType,
    targetId,
    includeImages,
    uploadMedia: settings.uploadMedia,
    aiResult: ai.result,
