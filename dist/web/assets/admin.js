...
var methodSubmitButton = document.getElementById("method-submit");
var methodCancelButton = document.getElementById("method-cancel");
...
var runtimeSecretState = null;
var activeRuntimeDatabase = null;
var collector = null;
...
function formatActiveDatabaseSummary(database) {
  if (!database) {
    return "\u2014";
  }
  if (database.backend === "postgres") {
    return `Postgres \xB7 ${database.tableName || "threads_web_store"} \xB7 ${database.storeKey || "default"}`;
  }
...
function beginMethodEdit(method) {
  if (!methodForm) {
    return;
  }
...
function parseStorefrontFaqs(raw) {
...
function parseStorefrontForm() {
  ...
  return {
    ...
    heroNotes: (formData.get("heroNotes")?.toString() ?? "").split("\n").map((line) => line.trim()).filter(Boolean),
    faqs: parseStorefrontFaqs(formData.get("faqs")?.toString() ?? "")
  };
}
function populateStorefrontForm(settings) {
...
function parseRuntimeConfigFromForm() {
  if (!runtimeForm) {
    return {};
  }
...
function populateRuntimeForm(config) {
  ...
  const applySecretPlaceholder = (name, configured, configuredPlaceholder) => {
    const field = runtimeForm.elements.namedItem(name);
...
function populateCollectorForm(config) {
...
async function refreshAdminData() {
  const [nextDashboard, runtimeResponse] = await Promise.all([
    requestAdmin("/api/admin/dashboard"),
    requestAdmin("/api/admin/runtime-config")
  ]);
